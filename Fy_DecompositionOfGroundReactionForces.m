...
clear R_FYG
clear R_FYD
R_FYG = zeros(sizePTF,1);
R_FYD = zeros(sizePTF,1);


if PiedGauche > PiedDroit % détection début (pied G ou D)
  for var1 =1:4:sizelim-1,
    R_FYG(Lim(var1):Lim(var1+1)) = R_FY(Lim(var1):Lim(var1+1),1);
    R_FYD(Lim(var1):Lim(var1+1)) = zeros(Lim(var1+1)-Lim(var1)+1,1);
  end
  for var1 =2:4:sizelim-1,
    R_FYG(Lim(var1)+0:Lim(var1+1)-1) = (-R_FY(Lim(var1)+0,1))*([Lim(var1)+0:Lim(var1+1)-1]-Lim(var1+1)) / (Lim(var1+1)-(Lim(var1)+0));
    R_FYD(Lim(var1)+0:Lim(var1+1)-1) = (R_FY(Lim(var1+1),1))*([Lim(var1)+0:Lim(var1+1)-1]-(Lim(var1)+0)) / (Lim(var1+1)-(Lim(var1)+0));  
  end
  for var1 =3:4:sizelim-1,
    R_FYD(Lim(var1):Lim(var1+1)) = R_FY(Lim(var1):Lim(var1+1),1);
    R_FYG(Lim(var1):Lim(var1+1)) = zeros(Lim(var1+1)-Lim(var1)+1,1);
  end
  for var1 =4:4:sizelim-1,
    if var1 < 8,
    R_FYD(Lim(var1)+0:Lim(var1+1)-1) = (-R_FY(Lim(var1)+0,1))*([Lim(var1)+0:Lim(var1+1)-1]-Lim(var1+1)) / (Lim(var1+1)-(Lim(var1)+0)); 
    R_FYG(Lim(var1)+0:Lim(var1+1)-1) = (R_FY(Lim(var1+1),1))*([Lim(var1)+0:Lim(var1+1)-1]-(Lim(var1)+0)) / (Lim(var1+1)-(Lim(var1)+0));
    end
...
sizeR_FYG = size(R_FYG,1);
sizeR_FYD = sizeR_FYG; % = size(R_FZD,1)-1;
...
hauteur_fleche = max(Fy_gauche(Lim(2)-20:Lim(3)+20))+3;
hauteur_texte = max(Fy_gauche(Lim(2)-20:Lim(3)+20))+5;
distance_fleche = 0.9;
...
for var1 = Lim(2)-20:1:Lim(3)-1, 
    line([var1,var1+1]/frequency,[Fy_gauche(var1),Fy_gauche(var1+1)],'color','k','LineStyle','-','Linewidth',1.35); %,'Linewidth',5);
    hold on;
end

for var1 = Lim(3):1:Lim(3)+20, 
     line([var1,var1+1]/frequency,[Fy_gauche(var1),Fy_gauche(var1+1)],'color','k','LineStyle','-','Linewidth',1.35);
    hold on;
end
...
spline_R_FYD = zeros(1,echantillons);
spline_R_FYG = zeros(1,echantillons);

clear spli_R_FYG
for var1 = 1:size(spline_xa,2),
    spli_R_FYG(var1) = R_FYG(spline_xa(var1));
end

spline_R_FYG(Lim(1):Lim(1+3)) = spline(spline_xa,spli_R_FYG,Lim(1):Lim(1+3));
spline_R_FYD(Lim(1):Lim(1+3)) = R_FY(Lim(1):Lim(1+3))'- spline_R_FYG(Lim(1):Lim(1+3));

% affichage interpolation spline :
for var1 = Lim(2):1:Lim(3)-1, 
    line([var1,var1+1]/frequency,[spline_R_FYG(var1),spline_R_FYG(var1+1)],'color','m','LineStyle',':','Linewidth',1.2);
    hold on;
end
...
if PiedDroit == 1, % choisir bonne courbe !
    spline_R_FYG2 = spline_R_FYD;
    spline_R_FYD2 = spline_R_FYG;
    spline_R_FYD = spline_R_FYD2;
    spline_R_FyG = spline_R_FYG2;
end
% Lim(1:7) = Lim(2:8);
    transition = 1;
    % Transition 1 :
    errorabs_G1 = abs (Fy_gauche(Lim(2):Lim(3)) - spline_R_FYG(Lim(2):Lim(3))');
    max_force1 = max( Fy_gauche(Lim(2):Lim(3)) + Fy_droite(Lim(2):Lim(3))) ; % JMNI
    error_G1 = abs ( errorabs_G1 / max_force1 ); % JMNI
    mean_errorG1 = mean(error_G1);
    cov_errorG1 = cov(error_G1);
    errorabs_D1 = abs (Fy_droite(Lim(2):Lim(3)) - spline_R_FYD(Lim(2):Lim(3))');
    ...
    max_forceY=(max_force1); %+max_force2)/2;
    errorabs_Y_Spline = (mean(errorabs_G1)+mean(errorabs_D1))/2; %+mean(errorabs_G2)+mean(errorabs_D2))/4; % err. totale approx.
    errorrel_Y_Spline = errorabs_Y_Spline/max_forceY*100 % err. rel.
toc
...
parD1 = Fy_gauche(Lim(2)); % Constraint 1: 1st point of Poly3_R_FXG is last point of the SS1
ndiff = 5; % number of points to calculate the slope on the ndiff last points of the SS1
parC1 = (Fy_gauche(Lim(2))-Fy_gauche(Lim(2)-ndiff-10))/(ndiff); % Constraint 2: slope of the 1st point of Poly3_R_FXG is slope of ndiff (5) last points of the SS1
LimEnd = Lim(3)-Lim(2); % Number of points of the DS1
parE1 = 0; %parE1 = -0.8; % Arbitrary slope at the end of DS1
...
Poly3_R_FYD = spline_R_FYD; % Initial condition. Poly3_R_FXD will be adapted further in the DS1, the rest being like spline_R_FXD.
Poly3_R_FYG = spline_R_FYG;

for Xpoly=1:LimEnd-2, % Creates the Poly3 during the DS1
    Poly3_R_FYG(Lim(2)+Xpoly) = parA1*Xpoly^3 + parB1 * Xpoly^2 + parC1 * Xpoly + parD1;
end
Poly3_R_FYD(Lim(2)+1:LimEnd-2) = R_FY(Lim(2)+1:LimEnd-2)'- Poly3_R_FYG(Lim(2)+1:LimEnd-2); % Force on other foot = Ftot - force on this foot.


spline_R_FYG = Poly3_R_FYG;
spline_R_FYD = Poly3_R_FYD;

% affichage Poly3 :
for var1 = Lim(2):1:Lim(3)-1, 
    line([var1,var1+1]/frequency,[Poly3_R_FYG(var1),Poly3_R_FYG(var1+1)],'color','r','LineStyle',':','Linewidth',1.2);
    hold on;
end
...
% Lim(1:7) = Lim(2:8);
    ...
    max_forceY=(max_force1); %+max_force2)/2;
    errorabs_Y_Poly3 = (mean(errorabs_G1)+mean(errorabs_D1))/2; %+mean(errorabs_G2)+mean(errorabs_D2))/4; % err. totale approx.
    errorrel_Y_Poly3 = errorabs_Y_Poly3/max_forceY*100 % err. totale approx.
toc
...
tic
FSS=Fy_gauche(Lim(2));
TDS=((Lim(3)+2)-Lim(2))/100;


a=FSS*0.6; 
b=2*TDS;
c=((Lim(2)-20)/100)-(TDS/2);%c=((Lim(2)+20)/100)-(TDS*0.55);
d=FSS*0.5;  %d=FSS*0.45; 


for var1 = Lim(2):1:Lim(3), 
    Sine_Fy_G(var1) = a*sin(((var1/100-c)*2*pi)/b)+d;     
end


Sine_Fy_D(Lim(2):1:Lim(3)) = R_FY(Lim(2):Lim(3))'- Sine_Fy_G(Lim(2):Lim(3));

% % % % % % % % % % % % 
% % % Affichage Sinus
% % % % % % % % % % % % 
for var1 = Lim(2):1:Lim(3)-1,
    line([var1,var1+1]/frequency,[Sine_Fy_G(var1),Sine_Fy_G(var1+1)],'color','[0 0.8 0]','LineStyle',':','Linewidth',1.2);
    hold on;
end
...
%-----------------------------------------------------
transition=1;
 errorabs_G1 = abs (Fy_gauche(Lim(2):Lim(3)) - Sine_Fy_G(Lim(2):Lim(3))');
            max_force1 = max( Fy_gauche(Lim(2):Lim(3)) + Fy_droite(Lim(2):Lim(3))) ; % JMNI
            error_G1 = abs ( errorabs_G1 / max_force1 ); % JMNI
            mean_errorG1 = mean(error_G1);
            cov_errorG1 = cov(error_G1);
            errorabs_D1 = abs (Fy_droite(Lim(2):Lim(3)) - Sine_Fy_D(Lim(2):Lim(3))');
            error_D1 = abs ( errorabs_D1 / max_force1 ); % JMNI
            mean_errorD1 = mean(error_D1);
            cov_errorD1 = cov(error_D1);
            erreur_moy(transition) = mean_errorG1;
            ...
            max_forceY=(max_force1); %+max_force2)/2;
            errorabs_Y_Sine = (mean(errorabs_G1)+mean(errorabs_D1))/2; %+mean(errorabs_G2)+mean(errorabs_D2))/4; % err. totale approx.
            errorrel_Y_Sine = errorabs_Y_Sine/max_forceY*100 % err. rel. 
toc
% % % % % % % % % % % % % % % % % % % % % % % % % % % 
% Décomposition des forces
% % % % % % % % % % % % % % % % % % % % % % % % % % % 
...
FSS=Fy_gauche(Lim(2));
TDS=(Lim(3)-Lim(2))/100; 
dFSS1=(Fy_gauche(Lim(2))-Fy_gauche(Lim(2)-1));
dE=(Fy_gauche(Lim(3))-Fy_gauche(Lim(3)-3)/3);
...
for var1 = Lim(2):1:Lim(3), 
    Sine_Fy_G(var1) = FSS/(1+exp(b*(var1-Lim(2)-100*(0.5*TDS)+100*(-abs(dFSS1)/4))/100));  %                              
end
%%% parameter ends%%
...
% % % % % % % % % % % % % % % % % % % % %
for var1 = Lim(2):1:Lim(3)-1,
    line([var1,var1+1]/frequency,[Sine_Fy_G(var1),Sine_Fy_G(var1+1)],'color','[1 0.5 0]','LineStyle',':','Linewidth',1.2);
    hold on;
end
...
transition=1;
 errorabs_G1 = abs (Fy_gauche(Lim(2):Lim(3)) - Sine_Fy_G(Lim(2):Lim(3))');
            ...
            max_forceY=(max_force1); 
            errorabs_Y_Sigmoid = (mean(errorabs_G1)+mean(errorabs_D1))/2;
            errorrel_Y_Sigmoid = errorabs_Y_Sigmoid/max_forceY*100 % err. rel.
...
for var1 = Lim(2):1:Lim(3), 
    Sine_Fy_G(var1) = FSS/(1+exp(b*(var1-Lim(2)-100*(0.5*TDS)+100*(-abs(dFSS1)/4))/100));  % 
    if (abs(Sine_Fy_G(var1)-Sine_Fy_G(var1-1))>(0.01))
        flat=[flat,var1];
    end
end
% % % Sine
for var1=Lim(2):1:flat(length(flat)),
    Sine_Fy_G(var1)=dFSS1*((flat(length(flat))-Lim(2))/3)*sin(pi*(var1-Lim(2))/((flat(length(flat))-Lim(2))))+Sine_Fy_G(var1);
end
...
% % % % % % % % % % % % % % % % % % % % %
for var1 = Lim(2):1:Lim(3)-1,
    line([var1,var1+1]/frequency,[Sine_Fy_G(var1),Sine_Fy_G(var1+1)],'color','b','LineStyle','-','Linewidth',1.35);
    hold on;
end
...
transition=1;
 errorabs_G1 = abs (Fy_gauche(Lim(2):Lim(3)) - Sine_Fy_G(Lim(2):Lim(3))');
            ...
            max_forceY=(max_force1); 
            errorabs_Y_Sine_Sigmoid = (mean(errorabs_G1)+mean(errorabs_D1))/2;
            errorrel_Y_Sine_Sigmoid = errorabs_Y_Sine_Sigmoid/max_forceY*100 % err. rel.

toc
% Write error
filename = 'Fy_Error.xlsx';
A = {'errorrel_Y_Spline','errorrel_Y_Poly3','errorrel_Y_Sine','errorrel_Y_Sigmoid','errorrel_Y_Sine_Sigmoid'; errorrel_Y_Spline, errorrel_Y_Poly3,errorrel_Y_Sine,errorrel_Y_Sigmoid,errorrel_Y_Sine_Sigmoid};
sheet = 1;
xlRange = 'A1';

B = {'errorabs_Y_Spline','errorabs_Y_Poly3','errorabs_Y_Sine','errorabs_Y_Sigmoid','errorabs_Y_Sine_Sigmoid'; errorabs_Y_Spline, errorabs_Y_Poly3,errorabs_Y_Sine,errorabs_Y_Sigmoid,errorabs_Y_Sine_Sigmoid};
sheet = 2;
xlRange = 'A1';
xlswrite(filename,B,sheet,xlRange)

% % % Legend of the figure
title('Antero-Posterior forces (F_Y)','FontName','arial','Fontsize',12)
xlabel('Time (s)','FontName','arial','Fontsize',10)
ylabel('Force (N)','FontName','arial','Fontsize',10)


axis([deb_aff fin_aff min(Fy_gauche(Lim(2)-20:Lim(3)+20))-20 max(Fy_gauche(Lim(2)-20:Lim(3)+20))+20]); %equal; 
