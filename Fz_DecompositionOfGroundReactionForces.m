...
clear R_FZG
clear R_FZD
R_FZG = zeros(sizePTF,1);
R_FZD = zeros(sizePTF,1);

if PiedGauche > PiedDroit % détection début (pied G ou D)
  for var1 =1:4:sizelim-1,
    R_FZG(Lim(var1):Lim(var1+1)) = R_FZ(Lim(var1):Lim(var1+1),1);
    R_FZD(Lim(var1):Lim(var1+1)) = zeros(Lim(var1+1)-Lim(var1)+1,1);
  end
  for var1 =2:4:sizelim-1,
    R_FZG(Lim(var1)+0:Lim(var1+1)-1) = (-R_FZ(Lim(var1)+0,1))*([Lim(var1)+0:Lim(var1+1)-1]-Lim(var1+1)) / (Lim(var1+1)-(Lim(var1)+0));
    R_FZD(Lim(var1)+0:Lim(var1+1)-1) = (R_FZ(Lim(var1+1),1))*([Lim(var1)+0:Lim(var1+1)-1]-(Lim(var1)+0)) / (Lim(var1+1)-(Lim(var1)+0));  
  end
  for var1 =3:4:sizelim-1,
    R_FZD(Lim(var1):Lim(var1+1)) = R_FZ(Lim(var1):Lim(var1+1),1);
    R_FZG(Lim(var1):Lim(var1+1)) = zeros(Lim(var1+1)-Lim(var1)+1,1);
  end
  for var1 =4:4:sizelim-1,
    if var1 < 8,
    R_FZD(Lim(var1)+0:Lim(var1+1)-1) = (-R_FZ(Lim(var1)+0,1))*([Lim(var1)+0:Lim(var1+1)-1]-Lim(var1+1)) / (Lim(var1+1)-(Lim(var1)+0)); 
    R_FZG(Lim(var1)+0:Lim(var1+1)-1) = (R_FZ(Lim(var1+1),1))*([Lim(var1)+0:Lim(var1+1)-1]-(Lim(var1)+0)) / (Lim(var1+1)-(Lim(var1)+0));
    end

  end
else
  for var1 = 1:4:sizelim-1, %(si pied D d'abord)
    R_FZD(Lim(var1):Lim(var1+1)) = R_FY(Lim(var1):Lim(var1+1),1);
    R_FYG(Lim(var1):Lim(var1+1)) = zeros(Lim(var1+1)-Lim(var1)+1,1);
  end
  for var1 = 2:4:sizelim-1,
    R_FYD(Lim(var1)+0:Lim(var1+1)-1) = (-R_FZ(Lim(var1)+0,1))*([Lim(var1)+0:Lim(var1+1)-1]-Lim(var1+1)) / (Lim(var1+1)-(Lim(var1)+0));  
    R_FZG(Lim(var1)+0:Lim(var1+1)-1) = (R_FZ(Lim(var1+1),1))*([Lim(var1)+0:Lim(var1+1)-1]-(Lim(var1)+0)) / (Lim(var1+1)-(Lim(var1)+0));  
  end
  for var1 = 3:4:sizelim-1,
    R_FZG(Lim(var1):Lim(var1+1)) = R_FZ(Lim(var1):Lim(var1+1),1);
...
sizeR_FZG = size(R_FZG,1);
sizeR_FZD = sizeR_FZG; % = size(R_FZD,1)-1;
...
hauteur_fleche = max(Fz_gauche(Lim(2)-20:Lim(3)+20))+3;
hauteur_texte = max(Fz_gauche(Lim(2)-20:Lim(3)+20))+20;
distance_fleche = 0.9;
...
for var1 = Lim(2)-20:1:Lim(3)-1, 
    line([var1,var1+1]/frequency,[Fz_gauche(var1),Fz_gauche(var1+1)],'color','k','LineStyle','-','Linewidth',1.35); %,'Linewidth',5);
    hold on;
end

for var1 = Lim(3):1:Lim(3)+20, 
     line([var1,var1+1]/frequency,[Fz_gauche(var1),Fz_gauche(var1+1)],'color','k','LineStyle','-','Linewidth',1.35);
    hold on;
end
...
spline_R_FZD = zeros(1,echantillons);
spline_R_FZG = zeros(1,echantillons);

clear spli_R_FZG
for var1 = 1:size(spline_xa,2),
    spli_R_FZG(var1) = R_FZG(spline_xa(var1));
end

spline_R_FZG(Lim(1):Lim(1+3)) = spline(spline_xa,spli_R_FZG,Lim(1):Lim(1+3));
spline_R_FZD(Lim(1):Lim(1+3)) = R_FZ(Lim(1):Lim(1+3))'- spline_R_FZG(Lim(1):Lim(1+3));

% affichage interpolation spline :
for var1 = Lim(2):1:Lim(3)-1, 
    line([var1,var1+1]/frequency,[spline_R_FZG(var1),spline_R_FZG(var1+1)],'color','m','LineStyle',':','Linewidth',1.2);
    hold on;
end
...
if PiedDroit == 1, % choisir bonne courbe !
    spline_R_FZG2 = spline_R_FZD;
    spline_R_FZD2 = spline_R_FZG;
    spline_R_FZD = spline_R_FZD2;
    spline_R_FZG = spline_R_FZG2;
end
% Lim(1:7) = Lim(2:8);
    transition = 1;
    % Transition 1 :
    errorabs_G1 = abs (Fz_gauche(Lim(2):Lim(3)) - spline_R_FZG(Lim(2):Lim(3))');
    max_force1 = max( Fz_gauche(Lim(2):Lim(3)) + Fz_droite(Lim(2):Lim(3))) ; % JMNI
    error_G1 = abs ( errorabs_G1 / max_force1 ); % JMNI
    mean_errorG1 = mean(error_G1);
    cov_errorG1 = cov(error_G1);
    errorabs_D1 = abs (Fz_droite(Lim(2):Lim(3)) - spline_R_FZD(Lim(2):Lim(3))');
    ...
    max_forceZ=(max_force1); %+max_force2)/2;
    errorabs_Z_Spline = (mean(errorabs_G1)+mean(errorabs_D1))/2; %+mean(errorabs_G2)+mean(errorabs_D2))/4; % err. totale approx.
    errorrel_Z_Spline = errorabs_Z_Spline/max_forceZ*100 % err. rel.
toc
...
parD1 = Fz_gauche(Lim(2)); % Constraint 1: 1st point of Poly3_R_FZG is last point of the SS1
ndiff = 5; % number of points to calculate the slope on the ndiff last points of the SS1
parC1 = (Fz_gauche(Lim(2))-Fz_gauche(Lim(2)-ndiff+1))/(ndiff); % Constraint 2: slope of the 1st point of Poly3_R_FZG is slope of ndiff (5) last points of the SS1
LimEnd = Lim(3)-Lim(2)+1; % Number of points of the DS1
parE1 = 0; %parE1 = -2; % Arbitrary slope at the end of DS1
...
Poly3_R_FZD = spline_R_FZD; % Initial condition. Poly3_R_FZD will be adapted further in the DS1, the rest being like spline_R_FZD.
Poly3_R_FZG = spline_R_FZG;

for Xpoly=Lim(1):LimEnd, % Creates the Poly3 during the DS1
    Poly3_R_FZG(Lim(2)+Xpoly) = parA1*Xpoly^3 + parB1 * Xpoly^2 + parC1 * Xpoly + parD1;
end
Poly3_R_FZD(Lim(2):LimEnd-2) = R_FZ(Lim(2):LimEnd-2)'- Poly3_R_FZG(Lim(2):LimEnd-2); % Force on other foot = Ftot - force on this foot.

% 
spline_R_FZG = Poly3_R_FZG;
spline_R_FZD = Poly3_R_FZD;

% affichage Poly3 :
for var1 = Lim(2):1:Lim(3)-1, 
    line([var1,var1+1]/frequency,[Poly3_R_FZG(var1),Poly3_R_FZG(var1+1)],'color','r','LineStyle',':','Linewidth',1.2);
    hold on;
end
...
% Lim(1:7) = Lim(2:8);
    ...
    max_forceZ=(max_force1); %+max_force2)/2;
    errorabs_Z_Poly3 = (mean(errorabs_G1)+mean(errorabs_D1))/2; %+mean(errorabs_G2)+mean(errorabs_D2))/4; % err. totale approx.
    errorrel_Z_Poly3 = errorabs_Z_Poly3/max_forceZ*100 % err. totale approx.
toc
...
for var1 = Lim(2):1:Lim(3), 
    Sine_Fz_G(var1) = a*sin(((var1/100-c)*2*pi)/b)+d;     
end



Sine_Fz_D(Lim(2):1:Lim(3)) = R_FZ(Lim(2):Lim(3))'- Sine_Fz_G(Lim(2):Lim(3));

% % % % % % % % % % % % 
% % % Affichage Sinus
% % % % % % % % % % % % 
for var1 = Lim(2):1:Lim(3)-1,
    line([var1,var1+1]/frequency,[Sine_Fz_G(var1),Sine_Fz_G(var1+1)],'color','[0 0.8 0]','LineStyle','-','Linewidth',1.35);
    hold on;
end
...
%-----------------------------------------------------
transition=1;
 errorabs_G1 = abs (Fz_gauche(Lim(2):Lim(3)) - Sine_Fz_G(Lim(2):Lim(3))');
            max_force1 = max( Fz_gauche(Lim(2):Lim(3)) + Fz_droite(Lim(2):Lim(3))) ; % JMNI
            error_G1 = abs ( errorabs_G1 / max_force1 ); % JMNI
            mean_errorG1 = mean(error_G1);
            cov_errorG1 = cov(error_G1);
            errorabs_D1 = abs (Fz_droite(Lim(2):Lim(3)) - Sine_Fz_D(Lim(2):Lim(3))');
            error_D1 = abs ( errorabs_D1 / max_force1 ); % JMNI
            mean_errorD1 = mean(error_D1);
            cov_errorD1 = cov(error_D1);
            erreur_moy(transition) = mean_errorG1;
            ...
            max_forceZ=(max_force1); %+max_force2)/2;
            errorabs_Z_Sine = (mean(errorabs_G1)+mean(errorabs_D1))/2; %+mean(errorabs_G2)+mean(errorabs_D2))/4; % err. totale approx.
            errorrel_Z_Sine = errorabs_Z_Sine/max_forceZ*100 % err. rel. 
toc
% % % % % % % % % % % % % % % % % % % % % % % % % % % 
% Décomposition des forces
% % % % % % % % % % % % % % % % % % % % % % % % % % % 
...
TDS=(Lim(3)-Lim(2))/100;
%%%% parameter starts%%%%
dFSS1=(Fz_gauche(Lim(2))-Fz_gauche(Lim(2)-1));
dE=(Fz_gauche(Lim(3))-Fz_gauche(Lim(3)-3)/3);
...
flat=[];
for var1 = Lim(2):1:Lim(3), 
    Sine_Fz_G(var1) = FSS/(1+exp(b*(var1-Lim(2)-100*(0.5*TDS)+100*(-abs(0)/4))/100));  %
    if (abs(Sine_Fz_G(var1)-Sine_Fz_G(var1-1))>(6.5));
        flat=[flat,var1];
    end
end
...
% % % % % % % % % % % % % % % % % % % % %
for var1 = Lim(2):1:Lim(3)-1,
    line([var1,var1+1]/frequency,[Sine_Fz_G(var1),Sine_Fz_G(var1+1)],'color','[1 0.5 0]','LineStyle',':','Linewidth',1.2);
    hold on;
end
...
transition=1;
 errorabs_G1 = abs (Fz_gauche(Lim(2):Lim(3)) - Sine_Fz_G(Lim(2):Lim(3))');
            ...
            max_forceZ=(max_force1); 
            errorabs_Z_Sigmoid = (mean(errorabs_G1)+mean(errorabs_D1))/2;
            errorrel_Z_Sigmoid = errorabs_Z_Sigmoid/max_forceZ*100 % err. rel.
...
for var1=Lim(2):1:flat(2),
    Sine_Fz_G(var1)=(dFSS1)*((flat(2)-Lim(2))/3)*sin(-0.5*pi+0.5*pi*(var1-Lim(2))/((flat(2)-Lim(2))))+Sine_Fz_G(var1);
end
display (flat(2)); display(Lim(2));
%%% parameter ends%%
display (flat(2)); display(Lim(2));
...
% % % % % % % % % % % % % % % % % % % % %
for var1 = Lim(2):1:Lim(3)-1,
    line([var1,var1+1]/frequency,[Sine_Fz_G(var1),Sine_Fz_G(var1+1)],'color','b','LineStyle',':','Linewidth',1.2);
    hold on;
end
...
transition=1;
 errorabs_G1 = abs (Fz_gauche(Lim(2):Lim(3)) - Sine_Fz_G(Lim(2):Lim(3))');
            ...
            max_forceZ=(max_force1); 
            errorabs_Z_Sine_Sigmoid = (mean(errorabs_G1)+mean(errorabs_D1))/2;
            errorrel_Z_Sine_Sigmoid = errorabs_Z_Sine_Sigmoid/max_forceZ*100 % err. rel.

toc
% Write error
filename = 'Fz_Error.xlsx';
A = {'errorrel_Z_Spline','errorrel_Z_Poly3','errorrel_Z_Sine','errorrel_Z_Sigmoid','errorrel_Z_Sine_Sigmoid'; errorrel_Z_Spline, errorrel_Z_Poly3,errorrel_Z_Sine,errorrel_Z_Sigmoid,errorrel_Z_Sine_Sigmoid};
sheet = 1;
xlRange = 'A1';
xlswrite(filename,A,sheet,xlRange)

B = {'errorabs_Z_Spline','errorabs_Z_Poly3','errorabs_Z_Sine','errorabs_Z_Sigmoid','errorabs_Z_Sine_Sigmoid'; errorabs_Z_Spline, errorabs_Z_Poly3,errorabs_Z_Sine,errorabs_Z_Sigmoid,errorabs_Z_Sine_Sigmoid};
sheet = 2;
xlRange = 'A1';
xlswrite(filename,B,sheet,xlRange)
% % % Legend of the figure
title('Vertical forces (F_Z)','FontName','arial','Fontsize',12)
xlabel('Time (s)','FontName','arial','Fontsize',10)
ylabel('Force (N)','FontName','arial','Fontsize',10)


axis([deb_aff fin_aff min(Fz_gauche(Lim(2)-20:Lim(3)+20))-20 max(Fz_gauche(Lim(2)-20:Lim(3)+20))+60]); %equal; 

h1 = plot(nan, nan, 'k-','Linewidth',1.35);
h2 = plot(nan, nan, 'm:','Linewidth',1.2);
h3 = plot(nan, nan,'r:','Linewidth',1.2); 
h4 = plot(nan, nan,'color',[0 0.8 0],'LineStyle','-','Linewidth',1.35); 
h5 = plot(nan, nan,'color',[1 0.5 0],'LineStyle',':','Linewidth',1.2);
h6 = plot(nan, nan, 'b:','Linewidth',1.2);
